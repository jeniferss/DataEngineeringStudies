...
if __name__ == '__main__':
    spark = SparkSession.builder.appName('Import Order Items').getOrCreate()

    BUCKET_NAME = 'js-s3files'
 
    input_path = "sparkairflow/landing/order_item.json"
    print(f"Importando dados do arquivo {input_path}")
    dataset = spark.read.json(f"s3a://{BUCKET_NAME}/{input_path}")
    dataset.show(5)

    output_path = 'sparkairflow/bronze/order_item.parquet'
